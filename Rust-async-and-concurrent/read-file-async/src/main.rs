...
#[tokio::main]
async fn main() {
    // tx is the sender for the function
    // rx is the receiver for main function
    let (tx, mut rx) = watch::channel(false);
    // here the channel has one producer (function) and multiple consumer (like main-function)
    tokio::spawn(watch_file_changes(tx));

    loop {
         let _ = rx.changed().await;
         if let Ok(contents) = read_file("data.txt").await {
            println!("Data has been changed: {}", contents);
         }
    }
}
