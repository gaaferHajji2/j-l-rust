...
fn process_pending(item: Pending, command: String, state: &Map<String, Value>) {
    ...
    match command.as_str() {
        "get" => item.get(&item.super_base.title, &state),
        "create" => item.create(&item.super_base.title, 
            &item.super_base.status.stringify(), &mut state),
        "edit" => item.set_to_done(&item.super_base.title, &mut state),
        _ => println!("check your command: {} for pending", command)
    }
}
